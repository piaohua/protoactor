...
func TestProto(t *testing.T) {
	packet := &messages.CLogin{}
	t.Logf("----:%+v", packet.String())
	s := proto.MessageName(&messages.CRegist{})
	t.Logf("s -> %s", s)
	//p := proto.MessageType(s)
	p := reflect.TypeOf(messages.CRegist{})
	t.Logf("p -> %v", p) //reflect.Type
	v := reflect.New(p)
	t.Logf("v -> %v", v)
	t.Logf("v -> %v", v.Interface())
	reg := &messages.CRegist{
		Phone: "1111",
	}
...
func TestRouter(t *testing.T) {
	reg := &messages.Request{
		UserName: "xxxx",
	}
	b, err := reg.Marshal()
	t.Log("err -> ", err)
	registTest(1000, messages.Request{})
	routerTest(1000, b)
}
...
func TestMarshal(t *testing.T) {
	...
	code, msg_b, err := packet(msg)
	t.Log(code, msg_b, err)
	//---
	msg1 := &messages.CRegist{
		Nickname: "xxxx",
		Phone:    "xxxx",
		Password: "xxxx",
	}
	code1, b1, err := packet(msg1)
	t.Log(code1, b1, err)
	//---
	msg2, err := unpack(code1, b1)
	t.Log(msg2, err)
}
